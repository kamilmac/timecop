...
pub fn render(frame: &mut Frame, area: Rect) {
    ...
    lines.push(Line::from(""));
    lines.push(section("Branch view (branch mode)"));
    lines.push(kv(",", "Toggle full-branch-diff ↔ uncommitted-only"));
    lines.push(Line::from(""));
    ...
    lines.push(kv("c", "Comment (PR: post immediately · branch: save draft)"));
    lines.push(kv("d", "Delete draft under cursor"));
    lines.push(kv("Y", "Yank all drafts to clipboard (branch mode)"));
    lines.push(Line::from(""));
    lines.push(section("Always available"));
    lines.push(kv("y", "Yank cursor target (code line / thread / draft)"));
